...
We are given a 2D Array task of size ‘N*3’ which tells us the merit point of specific activity on that particular day. Our task is to calculate the maximum number of merit task that the ninja can earn.
...
def solverec(day, task, last):
    ...
    for i in range(0, 3):
        if(i != last):
            point = task[day][i]+solverec(day-1, task, i)
            maxi = max(maxi, point)
    return maxi
...
def solveMemo(day, task, last, dp):
    ...
    for i in range(0, 3):
        if(i != last):
            point = task[day][i]+solveMemo(day-1, task, i, dp)
            maxi = max(maxi, point)
        dp[day][last] = maxi
    return dp[day][last]
...
def SolveTabulation(day, task, last, dp):
    ...
    for DaY in range(1, day+1):
        for LasT in range(4):
            for TasK in range(3):
                if(TasK != LasT):
                    point = dp[DaY-1][TasK] + task[DaY][TasK]
